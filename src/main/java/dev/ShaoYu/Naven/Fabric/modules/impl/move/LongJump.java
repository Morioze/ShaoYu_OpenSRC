...
public class LongJump extends Module {
   ...
   private final ModeValue useTrigger = ValueBuilder
      .create(this, "Use Trigger")
      .setModes("Mouse4", "Mouse5", "MiddleClick", "LeftClick", "KeyG")
      .setDefaultModeIndex(0)
      .build()
      .getModeValue();

   private final ModeValue releaseTrigger = ValueBuilder
      .create(this, "Release Trigger")
      .setModes("Mouse5", "Mouse4", "MiddleClick", "LeftClick", "KeyG")
      .setDefaultModeIndex(0)
      .build()
      .getModeValue();

   private boolean isUsePressed() {
      String m = this.useTrigger.getCurrentMode();
      long win = mc.getWindow().getWindow();
      return switch (m) {
         case "Mouse4" -> GLFW.glfwGetMouseButton(win, 3) == 1;
         case "Mouse5" -> GLFW.glfwGetMouseButton(win, 4) == 1;
         case "MiddleClick" -> GLFW.glfwGetMouseButton(win, 2) == 1;
         case "LeftClick" -> GLFW.glfwGetMouseButton(win, 0) == 1;
         case "KeyG" -> GLFW.glfwGetKey(win, GLFW.GLFW_KEY_G) == 1;
         default -> false;
      };
   }

   private boolean isReleasePressed() {
      String m = this.releaseTrigger.getCurrentMode();
      long win = mc.getWindow().getWindow();
      return switch (m) {
         case "Mouse5" -> GLFW.glfwGetMouseButton(win, 4) == 1;
         case "Mouse4" -> GLFW.glfwGetMouseButton(win, 3) == 1;
   ...
   private String getUseTriggerName() { return this.useTrigger.getCurrentMode(); }
   private String getReleaseTriggerName() { return this.releaseTrigger.getCurrentMode(); }
   ...
   public void onEnable() {
      ...
      ChatUtils.addChatMessage("§aLongJump enabled! Press " + getUseTriggerName() + " to jump & use fireball, " + getReleaseTriggerName() + " to release each knockback");
   }
   ...
   public void onUpdate(EventUpdate event) {
      if (this.isEnabled()) {
         if (this.shouldDisableAndRelease) {
            ...
            boolean currentUse = isUsePressed();
            if (currentUse && !this.usePressed) {
            ...
            boolean currentRelease = isReleasePressed();
            if (currentRelease && !this.releasePressed) {
               this.releasePressed = true;
      ...
      if (this.isEnabled()) {
         ...
         if (this.delayed) {
             ...
             statusText = String.format(
               "§eIntercepting: %d packets | Time: %.1fs | Press %s (%d/%d)",
               packetCount,
               (float)delayDuration / 1000.0F,
               getReleaseTriggerName(),
               this.releasedKnockbacks,
            ...
            statusText = "§bWaiting for input | " + getUseTriggerName() + ": Jump & use fireball | " + getReleaseTriggerName() + ": Release";
          }
      ...
      if (this.isEnabled() && mc.level != null) {
         if (this.delayed && event.getType() == EventType.RECEIVE) {
            ...
            if (event.getPacket() instanceof ClientboundSetEntityMotionPacket packet
               ...
               mc.execute(() -> ChatUtils.addChatMessage("§ePacket interception started, press " + getReleaseTriggerName() + " to release each"));
            }
         }
      } else {
         if (this.delayed) {
