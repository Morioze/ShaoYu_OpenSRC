...
public class Scaffold extends Module {
   ...
   public FloatValue rotationSpeed = ValueBuilder.create(this, "RotationSpeed")
   ...
   public FloatValue delayPlaceMs = ValueBuilder.create(this, "DelayPlace (ms)")
   ...
   public BooleanValue noFlickWhenJump = ValueBuilder.create(this, "NoFlickWhenJump")
   ...
   public BooleanValue blockCountDisplay = ValueBuilder.create(this, "BlockCount Display").setDefaultBooleanValue(true).build().getBooleanValue();
   public FloatValue blockCountX = ValueBuilder.create(this, "BlockCount X").setDefaultFloatValue(8.0F).setMinFloatValue(0.0F).setMaxFloatValue(10000.0F).setFloatStep(1.0F).build().getFloatValue();
   public FloatValue blockCountY = ValueBuilder.create(this, "BlockCount Y").setDefaultFloatValue(45.0F).setMinFloatValue(0.0F).setMaxFloatValue(10000.0F).setFloatStep(1.0F).build().getFloatValue();
   public FloatValue blockCountScale = ValueBuilder.create(this, "BlockCount Scale").setDefaultFloatValue(0.6F).setMinFloatValue(0.4F).setMaxFloatValue(2.0F).setFloatStep(0.05F).build().getFloatValue();
   ...
   public void onRender2D(EventRender2D e) {
      if (!this.blockCountDisplay.getCurrentValue()) return;
      ...
      // 绘制背景
      RenderUtils.drawRoundedRect(e.getStack(), x, y, width, height, 4.0F, new Color(0,0,0,90).getRGB());
      RenderUtils.fill(e.getStack(), x, y, x + width, y + 3.0F, new Color(120,20,20,220).getRGB());
   ...
   public void onEventEarlyTick(EventRunTicks e) {
      if (e.getType() == EventType.PRE && mc.screen == null && mc.player != null) {
         ...
         if (this.mode.isCurrentMode("Telly Bridge")) {
            if (this.offGroundTicks >= 1 && this.pos != null) {
         ...
         if (this.mode.isCurrentMode("Telly Bridge")) {
            ...
            if (this.offGroundTicks < 1 && PlayerUtils.movementInput() && !(this.noFlickWhenJump.getCurrentValue() && isHoldingJump)) {
               this.rots.setX(RotationUtils.rotateToYaw(this.rotationSpeed.getCurrentValue(), this.rots.getX(), mc.player.getYRot()));
   ...
   public void onClick(EventClick e) {
      e.setCancelled(true);
      if (mc.screen == null && mc.player != null && this.pos != null && (!this.mode.isCurrentMode("Telly Bridge") || (this.offGroundTicks >= 1 && this.tellyDelayElapsed()))) {
   ...
   private void placeBlock() {
      if (this.pos != null && isValidStack(mc.player.getMainHandItem())) {
         ...
         if (sbFace != null
                 ...
                 && this.shouldBuild()
                 && (!this.mode.isCurrentMode("Telly Bridge") || this.tellyDelayElapsed())) {
   ...
   private boolean tellyDelayElapsed() {
      if (!this.mode.isCurrentMode("Telly Bridge")) return true;
